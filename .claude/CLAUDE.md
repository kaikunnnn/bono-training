...
- `.claude/docs/analytics/GA-STATUS.md` — 実装済みイベント一覧・技術仕様
- `.claude/docs/analytics/GA-ANALYSIS-GUIDE.md` — 取得データの分析方法・KPI定義
- `.claude/docs/analytics/REPORT-SPEC.md` — 週次レポート仕様書

**`/report` コマンド**: GA4+Stripeから売上・継続レポートを生成
```
/report          → 直近7日のレポート
/report 月次     → 直近30日のレポート
/report 2026-03-01 2026-03-31 → 任意期間
```

**実装済み計測**:
- サブスクファネル: `view_plans` → `select_plan` → `begin_checkout` → `purchase`
- コンテンツ: `lesson_view`, `search`, `content_bookmarked`, `roadmap_view`
- ユーザー属性: `user_id`, `subscription_status`, `login`, `sign_up`
- Eコマース: GA4推奨 `purchase` イベント → Monetizationレポート自動生成

**解約データ**: Stripeポータル経由のためGA4では取得不可 → Stripe MCPで取得
...
├── skills/                             ← スキル（コマンド）
│   ├── design/                         ← /design コマンド
│   ├── report/                         ← /report コマンド（GA4週次レポート）
│   └── fast-implement/                 ← /fast-implement コマンド
└── docs/
    ├── analytics/                      ← GA4アナリティクス
    │   ├── GA-STATUS.md                ← 実装状況・イベント一覧
    │   ├── REPORT-SPEC.md              ← 週次レポート仕様書
    │   └── GA-ANALYSIS-GUIDE.md        ← 分析ガイド・KPI定義
    ├── features/                       ← ユーザー向け機能
