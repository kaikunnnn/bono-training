# データ移行プロジェクト

**目的**: 既存の Stripe 顧客・サブスクリプションデータ（2,162 人、250 件）を新システム（Supabase）に移行する

---

## 📚 ドキュメント構成

### 1. 📋 要件定義・ヒアリング

**[data-migration-planning.md](./data-migration-planning.md)**
- 移行前のヒアリングシート（29 問）
- 既に回答済み
- 移行規模、データ保存場所、紐付け方法などを定義

---

### 2. 📊 現状分析

**[current-implementation-summary.md](./current-implementation-summary.md)**
- 現在の Supabase テーブル構造
- 実装フロー
- テストデータの状況

**[current-data-status.md](./current-data-status.md)**
- Supabase の実データ状況（8 件のテストデータ）
- Stripe の顧客サンプル（2 件）
- 価格不一致問題の発見
- MemberStack メタデータの確認

---

### 3. 🎯 移行計画

**[migration-plan.md](./migration-plan.md)**
- 3 フェーズの移行戦略
- データ同期方法
- 並行稼働の手順
- リスク分析とロールバック

**[migration-next-steps.md](./migration-next-steps.md)**
- 実装手順の詳細版
- 3 つの移行スクリプト（コード付き）
- スケジュール（目安）
- チェックリスト

---

### 4. 🧪 テスト実行ガイド（★ 次にやること）

**[migration-test-guide.md](./migration-test-guide.md)**
- Phase 0: 1 件テスト（最初）
- Phase 1: 10 件テスト（次）
- Phase 2: 全件移行（最後）
- 動作確認方法
- トラブルシューティング

---

### 5. 📝 補足資料

**[information-checklist.md](./information-checklist.md)**
- データ収集のためのチェックリスト
- 既に完了

---

## 🚀 現在の状況

### ✅ 完了したこと

- [x] 要件ヒアリング
- [x] 現状分析
- [x] 移行計画の策定
- [x] CSV データのエクスポート
  - [x] `stripe-customers.csv` (2,162 件)
  - [x] `stripe-subscriptions.csv` (250 件前後)
- [x] テスト用 CSV の作成
  - [x] `stripe-customers-test.csv` (1 件)
  - [x] `stripe-subscriptions-test.csv` (1 件)
  - [x] `stripe-customers-10.csv` (10 件)
  - [x] `stripe-subscriptions-10.csv` (10 件)
- [x] Service Role Key の取得
- [x] 移行スクリプトの作成
  - [x] `scripts/migrate-create-auth-users.ts`
  - [x] `scripts/migrate-stripe-customers.ts`
  - [x] `scripts/migrate-subscriptions.ts`
- [x] **Phase 0: 1件テスト** ✅ 成功（2025-11-19）
- [x] **Phase 1: 10件テスト** ✅ 成功（2025-11-19）
  - 詳細: [phase1-test-report.md](./phase1-test-report.md)
- [x] **Phase 2: 全件移行（Auth ユーザー）** ✅ 成功（2025-11-28）
  - 詳細: [phase2-full-migration-report.md](./phase2-full-migration-report.md)
  - 結果: **2,022/2,162件** 成功（93.5%）
  - 実質エラー: **4件のみ**（0.18%）

### 🔄 次にやること

**Phase 2: 残りのテーブル同期**

1. ✅ **Script 1 完了**: Auth ユーザー作成（2,022/2,162件）

2. ⏸️ **Script 2 実行待ち**: stripe_customers テーブル同期
   ```bash
   npx tsx scripts/migrate-stripe-customers.ts stripe-customers.csv
   ```

3. ⏸️ **Script 3 実行待ち**: user_subscriptions テーブル同期
   ```bash
   npx tsx scripts/migrate-subscriptions.ts stripe-subscriptions.csv
   ```

4. ⏸️ **動作確認**
   - Supabase Dashboard でデータ確認
   - データ整合性チェック
   - （必要に応じて）パスワードリセットメール送信

---

## 📊 移行対象データ

### 既存システム

- **総顧客数**: 2,162 人
- **アクティブサブスクリプション**: 250 件
- **データソース**: Stripe + MemberStack
- **既存プラン**:
  - Standard 1 ヶ月: ¥6,800/月
  - Standard 3 ヶ月: ¥5,800/月
  - Feedback 1 ヶ月: ¥1,480/月
  - Feedback 3 ヶ月: ¥1,280/月

### 新システム

- **Supabase Auth**: ユーザー認証
- **Supabase Database**: データ保存
- **Stripe**: 既存のサブスクリプション ID を維持

---

## ⚠️ 重要な決定事項

### 1. 既存 Stripe サブスクリプション ID を維持

- 新しいサブスクリプションは作成しない
- 価格変更なし
- 課金継続

### 2. テストデータは残す

- 既存の 8 件のテストデータはクリアしない
- テスト環境として活用
- 本番データと混在しても問題なし（メタデータで区別）

### 3. 段階的移行

- Phase 0: 1 件テスト
- Phase 1: 10 件テスト
- Phase 2: 全件移行
- Phase 3: 並行稼働（1-2 週間）
- Phase 4: 完全移行

---

## 🔗 関連リンク

- [Stripe Dashboard](https://dashboard.stripe.com)
- [Supabase Dashboard](https://supabase.com/dashboard/project/fryogvfhymnpiqwssmuu)
- [プロジェクトルート](../../)

---

**最終更新**: 2025-11-19
**次回更新**: Phase 0 テスト完了後
