/**
 * Article Card Components - Type Definitions (統合版)
 * BONO UI/UXダッシュボード用
 */
...
/**
 * お気に入り記事カードのProps（ArticleCardを拡張）
 */
export interface FavoriteArticleCardProps extends ArticleCardProps {
  /**
   * お気に入り状態
   * @default false
   */
 ...
 * お気に入り記事データの型定義
 */
export interface FavoriteArticleData extends ArticleData {
  isFavorite: boolean;
}
 ...
 * 記事データのバリデーション
 */
export function validateArticleData(data: Partial<ArticleData>): data is ArticleData {
  return (
