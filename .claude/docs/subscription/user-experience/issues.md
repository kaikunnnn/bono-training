...
- [🔴 Critical（高優先度）](#-critical高優先度)
  - [[ISSUE-001] プラン変更時に二重課金が発生する](#issue-001-プラン変更時に二重課金が発生する)
  - [[ISSUE-002] 決済完了後、画面が更新されない](#issue-002-決済完了後画面が更新されない)
...
**問題**:
スタンダード → フィードバックにプラン変更したところ、両方のプランが課金されている。

**再現手順**:
1. スタンダードプランに登録
2. 「フィードバックプランを選択」ボタンをクリック
3. Stripe Checkoutで決済完了
4. Stripe Dashboardを確認 → 2つのアクティブなサブスクリプション

**期待値**:
- 古いスタンダードプランは自動的にキャンセルされる
- フィードバックプランのみアクティブになる
