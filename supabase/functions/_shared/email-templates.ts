/**
 * BONO メールテンプレート
 * Resend経由で送信するHTMLメールテンプレートを生成
 */

// BONOロゴSVG（Base64エンコード用）
...
export function getPlanDisplayName(planType: string, duration: number): string {
  ...
  const baseName = planNames[planType] || 'スタンダードプラン';
  return `${baseName}（${duration}ヶ月）`;
}
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 640px; background-color: #FFFFFF; border-radius: 32px; overflow: hidden; border: 1px solid #F5F5F5; box-shadow: 0px 4px 8px 0px rgba(0, 0, 0, 0.04);">
          ...
          <tr>
            <td style="padding: 32px 48px 24px 48px; text-align: left;">
              <img src="data:image/svg+xml;base64,${LOGO_BASE64}" alt="BONO" width="68" height="20" style="display: block;" />
            </td>
          </tr>
          <!-- コンテンツ -->
          <tr>
...
${content}
          ...
          <tr>
            <td style="padding: 32px 48px;">
              <img src="data:image/svg+xml;base64,${LOGO_BASE64}" alt="BONO" width="68" height="20" style="display: block; margin-bottom: 32px;" />
...
/**
 * ウェルカムメール（メンバーシップ登録完了）
 */
export function generateWelcomeEmail(): { subject: string; html: string } {
  const content = `
  ...
  return {
    subject: 'BONOへようこそ！メンバーシップ登録が完了しました',
 ...
 * 解約完了メール
 */
export function generateCancellationEmail(): { subject: string; html: string } {
  const content = `
              <h1 style="margin: 0 0 24px 0; font-size: 32px; font-weight: 700; color: #222222; line-height: 36px; text-align: left;">
                解約手続きが完了しました
              </h1>
              <p style="margin: 0 0 24px 0; font-size: 18px; color: #222222; line-height: 28px; text-align: left; font-weight: 400;">
                BONOのメンバーシッププランの解約登録が完了されました。<br><br>ご利用ありがとうございました。
              </p>
 ...
 * プラン変更完了メール
 */
export function generatePlanChangeEmail(planName: string): { subject: string; html: string } {
  const content = `
              <h1 style="margin: 0 0 24px 0; font-size: 32px; font-weight: 700; color: #222222; line-height: 36px; text-align: left;">
                プランが変更されました
              </h1>
              <p style="margin: 0 0 24px 0; font-size: 18px; color: #222222; line-height: 28px; text-align: left; font-weight: 400;">
                BONOのメンバーシッププランが変更されました。<br><br>【変更後のプラン】<br><strong style="font-size: 20px;">${planName}</strong>
              </p>

              <table cellpadding="0" cellspacing="0" style="margin-bottom: 24px;">
                <tr>
                  <td>
                    <a href="https://www.bo-no.design/mypage" style="display: inline-block; padding: 14px 24px; background-color: #151834; color: #FFFFFF; font-size: 16px; font-weight: 600; text-decoration: none; border-radius: 8px;">
                      マイページを確認する
                    </a>
                  </td>
                </tr>
  ...
  return {
    subject: 'BONOメンバーシップのプランが変更されました',
    html: baseTemplate(content),
  };
}
