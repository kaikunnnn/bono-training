...
export function BlogRichText({
  ...
  const standaloneUrls = useMemo(() => {
    ...
    for (const b of content) {
      if ((b as { _type?: string })?._type !== 'block') continue
      if ((b as { style?: string })?.style && (b as { style?: string }).style !== 'normal') continue
    ...
    () => ({
      ...
      types: {
        ...
        tableOfContents: ({ value }) => {
          const tocTitle = (value as { title?: string })?.title || '目次'
          const tocMaxDepth = (value as { maxDepth?: number })?.maxDepth || 2
