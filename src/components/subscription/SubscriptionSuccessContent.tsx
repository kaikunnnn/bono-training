 ...
 */
export const SubscriptionSuccessContent: React.FC<
  SubscriptionSuccessContentProps
> = ({ type, planType, duration, isLoading, error }) => {
  ...
  const texts = {
    new: {
      title: "登録が完了しました",
      subtitle: planType
        ? `${getFullPlanName(planType, duration)}へようこそ！`
        : "",
      description:
        "ご登録ありがとうございます！以下のコンテンツにアクセスできるようになりました。",
      loadingText: "決済情報を確認しています...",
    },
    updated: {
      title: "プラン変更が完了しました",
      subtitle: planType
        ? `${getFullPlanName(planType, duration)}への変更が完了しました`
        : "",
      description:
        "新しいプランが適用されました。引き続きサービスをお楽しみください。",
  ...
  if (error) {
    return (
      <div className="container py-16 max-w-2xl">
        <Card>
          <CardHeader>
            <CardTitle className="text-center font-noto-sans-jp">
              エラーが発生しました
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {type === "new" && (
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      window.open(LEGACY_SITE_LOGIN_URL, "_blank")
                    }
                  >
