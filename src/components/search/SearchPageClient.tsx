...
const ChatInterface = dynamic(() => import("@/components/ai/ChatInterface"), {
  ...
  loading: () => (
    <div className="flex h-full items-center justify-center">
      ...
      <span className="ml-3 text-gray-600">AIチャットを読み込み中...</span>
    </div>
  ),
});
...
function SearchPageInner({ initialData }: SearchPageClientProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const tab = resolveTab(searchParams);
  const queryParam = searchParams?.get("q") || "";
  const [hasOpenedAi, setHasOpenedAi] = useState(tab === "ai");
  const shouldRenderAi = hasOpenedAi || tab === "ai";
  ...
  const STEP = 5;
  const visibilityKey = `${tab}:${query}`;
  const [visibilityState, setVisibilityState] = useState<{
    key: string;
    counts: Partial<Record<SectionKey, number>>;
  }>({ key: visibilityKey, counts: {} });
  const visibleCount =
    visibilityState.key === visibilityKey ? visibilityState.counts : {};

  const getVisibleCount = (key: SectionKey) =>
    visibleCount[key] ?? INITIAL_VISIBLE;

  const showMore = useCallback((key: SectionKey) => {
    setVisibilityState((prev) => {
      const counts = prev.key === visibilityKey ? prev.counts : {};
      return {
        key: visibilityKey,
        counts: {
          ...counts,
          [key]: (counts[key] ?? INITIAL_VISIBLE) + STEP,
        },
      };
    });
  }, [visibilityKey]);
    ...
    <div className={cn("flex flex-col", tab === "ai" && "h-[calc(100vh-64px)]")}>
      ...
      <div className="px-4 pt-8 sm:pt-12">
        <div className="max-w-4xl mx-auto">
          ...
          <div
            className={cn(
              "grid overflow-hidden transition-[grid-template-rows,opacity,margin-bottom] duration-300 ease-in-out",
              tab !== "ai"
      ...
      {tab !== "ai" && (
        <div className="max-w-4xl mx-auto w-full px-4 py-8">
          {isLoading && <SearchResultsSkeleton tab={tab} />}
      ...
      {/* AI チャット: 初回にだけ遅延ロードし、以後は非表示でも履歴を保持 */}
      {shouldRenderAi && (
