...
/**
 * /dev/top4 — 余白統一版（top3 ベースの余白実験ページ）
 *
 * 内容・データ取得は /dev/top3 と完全に同一。違いは「ブロック間の外側の上下余白」だけ。
 * ユーザーが最も好んだ「みんなの実績」（AchievementHighlightSection の py-[120px]）の
 * ゆったりしたリズムを基準値として採用し、全ブロックの外側の上下パディングを
 * `paddingY={120}` で統一する。
 *
 * 各ブロックコンポーネントに追加した `paddingY` プロップは inline style として
 * 上下パディングを上書きするため、Tailwind のクラス順序に依存せず確実に効く。
 * top3 / top2 はこのプロップを渡さないので一切影響を受けない（before/after 比較用に残す）。
 *
 * /dev 配下なので本番では 404（dev/layout.tsx でゲート）。
 */

export const metadata: Metadata = {
  title: "余白統一版 (/dev/top4)",
  robots: { index: false, follow: false },
};

export default async function DevTop4Page() {
  ...
  return (
    ...
    <div className="container">
      <div className="flex flex-col">
        ...
        <EyecatchHero compact paddingY={120} />
        ...
        <FeatureLinkGrid compact paddingY={120} />
