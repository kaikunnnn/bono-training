/**
 * セクション4の記事2を、確認用のSanity development datasetへ反映する。
...
const ARTICLE_ID = "sbd-section4-dev-next-cycle";
const TITLE = "トレーニングを完走しよう";
const SOURCE_PATH =
  "/Users/kaitakumi/Documents/01_BONO事業/01_Docs/rebono/ペルソナと課題でデザインするフロー/Step4-セクション4をどうするのか/2.トレーニングを完走しよう.md";
...
function block(text: string, style = "normal", listItem?: "bullet" | "number"): Block {
  ...
  for (const m of text.matchAll(linkRe)) {
    ...
    if (start > cursor) children.push(...spans(text.slice(cursor, start)));
    const defKey = key();
    markDefs.push({ _key: defKey, _type: "link", href: m[2] });
    children.push({ _type: "span", _key: key(), marks: [defKey], text: m[1] });
    cursor = start + m[0].length;
  }
  if (cursor < text.length) children.push(...spans(text.slice(cursor)));
  return {
    _type: "block",
    _key: key(),
    style,
    markDefs: markDefs as unknown as [],
    children: children.length ? children : spans(text),
